...
## --------------------- Modeling ---------------------------- ##
def train_model(X_train, y_train, X_test, y_test, plot_name, n_estimators, max_depth):
    mlflow.set_experiment('CarPrices-processing')
    with mlflow.start_run() as run:
        mlflow.set_tag('classifier', 'gradient_boosting')

        # Train Gradient Boosting model
        clf = GradientBoostingClassifier(n_estimators=n_estimators, max_depth=max_depth, random_state=45)
...
def main(n_estimators: int, max_depth: int):
    ...
    train_model(X_train=X_train_balanced, y_train=y_train_balanced, X_test=X_test, y_test=y_test,
                plot_name='balanced_data', n_estimators=n_estimators, max_depth=max_depth)

    # Train the model with original imbalanced data
    train_model(X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test,
                plot_name='imbalanced_data', n_estimators=n_estimators, max_depth=max_depth)
