...
## --------------------- Modeling ---------------------------- ##
def train_model(X_train, y_train, X_test, y_test, plot_name, n, lr, d):
    mlflow.set_experiment('CarPrices-processing')
    with mlflow.start_run() as run:
        mlflow.set_tag('classifier', 'xgboost')  # Change classifier tag to XGBoost

        # Train XGBoost model with provided parameters
        clf = xgb.XGBClassifier(n_estimators=n, learning_rate=lr, max_depth=d, random_state=45)
        ...
        mlflow.log_params({'n_estimators': n, 'learning_rate': lr, 'max_depth': d})
...
if __name__ == '__main__':
    ...
    main(n=args.n, lr=args.lr, d=args.d)
